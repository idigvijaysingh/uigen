...
function getToolLabel(toolName: string, args: any): string {
  ...
  if (toolName === "str_replace_editor") {
    const verb =
      args?.command === "create"      ? "Creating" :
      args?.command === "str_replace" ? "Updating" :
      args?.command === "insert"      ? "Adding to" :
      args?.command === "view"        ? "Reading" :
                                        "Editing";
  ...
  if (toolName === "file_manager") {
    if (args?.command === "rename") {
      const newFile = args?.new_path?.split("/").pop() ?? "file";
      return `Renaming ${filename} → ${newFile}`;
    }
  ...
  if (messages.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full px-4 text-center">
        ...
        <p className="text-neutral-900 font-semibold text-lg mb-2">What would you like to build today?</p>
        <p className="text-neutral-500 text-sm max-w-sm">Describe any UI component and I'll generate it instantly — try something like <span className="text-blue-500 font-medium">"a login form with email and password"</span> or <span className="text-blue-500 font-medium">"a pricing card with a gradient"</span>.</p>
      </div>
    );
  }
